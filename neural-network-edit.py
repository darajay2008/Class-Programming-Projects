...
global noEpochs
global meanError
noEpochs = open("NumberOfEpochs.txt", "w")
meanError = open("MeanSquaredError.txt", "w")
 
for i in range(10000):
	training_inputs, training_outputs = random.choice(training_sets)
	nn.train(training_inputs, training_outputs)
    #print(i, nn.calculate_total_error(training_sets))
	noEpochs.write(str(i) + "\r\n")
	noEpochs.write("\n")
	meanError.write(str(nn.calculate_total_error(training_sets)) + "\r\n")
	meanError.write("\n")
...
for j in range(len(training_sets)):
	outputs.append(nn.feed_forward(training_sets[j][0]))
	#if outputs[j] >= 0.8:
	#	outputs[j] = 1
	#else:
	#	outputs[j] = 0
	...
	if got[0] >= 0.8:
		...
		got[0] = -1
	print('Expected = %d, Got = %d' % (expected[0], got[0]))
nn.inspect()
